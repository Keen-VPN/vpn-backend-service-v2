...
import { ApiOkResponse, ApiCreatedResponse, ApiBadRequestResponse, ApiUnauthorizedResponse, ApiForbiddenResponse, ApiInternalServerErrorResponse, ApiNotFoundResponse, getSchemaPath } from '@nestjs/swagger';
...
export const ApiStandardResponse = <TModel extends Type<any>>(options?: {
    ...
    const successDecorator = options?.status === 201
        ? ApiCreatedResponse
        : ApiOkResponse; // Default to 200

    const decorators = [
        ApiBadRequestResponse({ description: 'Bad Request (Validation failed)', type: ApiErrorResponseDto }),
